...
async def send_result(message: Message, state: FSMContext):
    ...
    if str(data["confirm"]) == "Да":
        ...
        if get_state_of_event_mode() is True:
            ...
            print(f"Админ: @{message.from_user.username} ID: {message.from_user.id} запустил режим мероприятия")
            await message.answer("✅ Режим мероприятия успешно включён", reply_markup=rmk)
        else:
            print(f"Админ: @{message.from_user.username} ID: {message.from_user.id} выключил режим мероприятия")
